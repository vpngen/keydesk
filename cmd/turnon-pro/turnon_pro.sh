...
EXECUTABLE_PATH="/opt/vgkeydesk/turnon-pro"
...
if [ -d "${DB_DIR}" ]; then
        if [ "${action}" = "-on" ];  then
                touch "${DB_DIR}/.pro" || fatal "500" "Internal server error" "Can't create .pro file"
        fi

        if [ -z "${DEBUG}" ]; then
                if id "${brigade_id}" >/dev/null 2>&1; then
                        sudo -u "${brigade_id}" "${EXECUTABLE_PATH}" "${action}" >&2 || fatal "500" "Internal server error" "Can't proize brigade"
                fi
        else
                ...
                if [ -x "${EXECUTABLE_PATH}" ]; then
                        ...
                        "${EXECUTABLE_PATH}" -id "${brigade_id}" -d "${DB_DIR}" "${action}" >&2 || fatal "500" "Internal server error" "Can't proize brigade"
                elif [ -s "${SOURCE_DIR}/main.go" ]; then
                        # shellcheck disable=SC2086
                        go run "${SOURCE_DIR}/" -id "${brigade_id}" -d "${DB_DIR}" "${action}" >&2 || fatal "500" "Internal server error" "Can't proize brigade"
                else 
        ...
        if [ "${action}" = "-off" ]; then
                rm -f "${DB_DIR}/.pro" || fatal "500" "Internal server error" "Can't remove .pro file"
        fi
else
