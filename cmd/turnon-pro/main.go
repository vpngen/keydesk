...
func parseArgs() (bool, bool, string, string, error) {
	...
	actionOn := flag.Bool("on", false, "Turn PRO on")
	actionOff := flag.Bool("off", false, "Turn PRO off")
...
func Do(db *storage.BrigadeStorage, on, off bool) error {
	...
	if on {
		if err := db.SetPRO(true); err != nil {
			return fmt.Errorf("set PRO: %w", err)
		}
	}

	if off {
		if err := db.SetPRO(false); err != nil {
			return fmt.Errorf("unset PRO: %w", err)
		}
	}
